...
export default function CreateAlbumModal({ onClose }) {
  ...
  useEffect(() => {
    // Fetch artist's existing approved and pending tracks
    const fetchMyTracks = async () => {
      try {
        const data = await fetchMyUploads();
        setExistingTracks(data.songs || []);
      } catch (err) {
        console.error('Failed to fetch existing tracks', err);
      }
    };
    fetchMyTracks();
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setStatusText('Creating album playlist...');
      const data = new FormData();
      data.append('name', formData.name);
      data.append('description', formData.description || 'New Album');
      ...
      // 2. Create the Album
      const response = await authApi.post('/api/albums', data, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });

      showSuccessToast("Album submitted for moderation!");
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 animate-fade-in">
      ...
      <div className="relative w-full max-w-2xl bg-white dark:bg-gray-900 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-gray-50/50 dark:bg-gray-800/50">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Create Album</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Group your uploaded tracks into an album for moderation.</p>
          </div>
          ...
          >
            {isSubmitting ? (
              <>
                ...
                <span>{statusText || 'Submitting...'}</span>
              </>
            ) : (
